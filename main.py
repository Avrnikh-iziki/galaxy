...
class MainWidget(RelativeLayout):
    from userAction import _on_keyboard_down, _on_keyboard_up, _keyboard_closed,  on_touch_down, on_touch_up
    from transformes import transform, transform_perspective, transform_2d
    ...
    menu_title = StringProperty("A   V   R   N   I   K   H")
    ...
    def check_ship_collision(self):
        for i in range(0, len(self.tiles_coordinates)):
            ...
            if ti_y > self.current_y_lop + 1:
                return False
            if self.chek_ship_collision_with_tile(ti_x, ti_y):
                return True
        return True

    def chek_ship_collision_with_tile(self, ti_x, ti_y):
    ...
    def init_vertical_lines(self):
        with self.canvas:
            Color(1, 1, 1)
            # self.Line = Line(points=[100, 0, 0, 100])
    ...
    def update(self, dt):
        ...
        if not self.state_game_over and self.state_game_has_started:
            speed_y = self.SPED * self.height / 100
            ...
            while self.current_offset_y >= spacing_y:
                ...
                self.score_txt = "SCORE:" + str(self.current_y_lop)
                self.generate_tiles_coordinates()
            speed_y = self.current_sped_x * self.width / 100
            self.current_offset_x += speed_y * time_factor
        ...
        if not self.check_ship_collision() and not self.state_game_over:
            ...
            Clock.schedule_once(self.play_gameover_impact_play, 3)
            print("game over")

    def play_gameover_impact_play(self, dt):
